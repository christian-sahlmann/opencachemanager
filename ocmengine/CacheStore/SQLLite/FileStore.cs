	...
	public partial class FileStore : ACacheStore
	{
		...
		private string DoPrefilter(string[] ownerIDs)
		{
			...
			if (AdvancedFilters.Contains(FilterList.KEY_INCNOATTRS) || AdvancedFilters.Contains(FilterList.KEY_EXCNOATTRS) ||
			    ...
			    AdvancedFilters.Contains(FilterList.KEY_FOUNDON) || (AdvancedFilters.Contains(FilterList.KEY_FOUNDON)))
			{
		...
		private String BuildWhereClause (Hashtable m_criteria, string[] ownerIDs)
		{
			...
			if (m_criteria.Contains (FilterList.KEY_U1)) {
				builder.Append (" AND Geocache.user1 LIKE '%" + (m_criteria[FilterList.KEY_U1] as string) + "%'");
			}
			if (m_criteria.Contains (FilterList.KEY_U2)) {
				builder.Append (" AND Geocache.user2 LIKE '%" + (m_criteria[FilterList.KEY_U2] as string) + "%'");
			}
			if (m_criteria.Contains (FilterList.KEY_U3)) {
				builder.Append (" AND Geocache.user3 LIKE '%" + (m_criteria[FilterList.KEY_U3] as string) + "%'");
			}
			if (m_criteria.Contains (FilterList.KEY_U4)) {
				builder.Append (" AND Geocache.user4 LIKE '%" + (m_criteria[FilterList.KEY_U4] as string) + "%'");
			}
			...
			if (m_criteria.Contains (FilterList.KEY_CACHE_NAME)) {
				builder.Append (" AND (Geocache.name LIKE '%" + m_criteria[FilterList.KEY_CACHE_NAME] + "%'");
				builder.Append ("OR Geocache.fullname LIKE '%" + m_criteria[FilterList.KEY_CACHE_NAME] + "%')");
			}
