...
namespace ocmengine.SQLLite
{
	...
	public partial class FileStore
	{
		...
		const string LAST_FOUND_ME_FILT = "SELECT MAX(date),cache from LOGS  WHERE (status='Found it' or status='find' or status='Attended' or status='Webcam Photo Taken') and (finderID IN ({0}) or loggedBy IN ({0})) GROUP BY cache";
